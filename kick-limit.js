...
let limit = args[0]
let limitt = db.fetch(`kicklimit_${message.guild.id}`)
if (!limit) return message.channel.send(new Discord.RichEmbed().setColor("FF0000").setDescription(`<a:ya:731500590949203999> | Lütfen Limiti Yaz Örnek: \`${prefix}kick-limit 1\``))
...
if (limit) message.channel.send(new Discord.RichEmbed().setColor("33FF00").setDescription(`<a:do:731500590945009694> | Kick Limit Sayısı: \`${limit}\` Olarak Ayarlandı`))
